...
class TestStrategy():
    ...
    def run(self):
        if self.method == 'random':
            ...
            self.random_wMemory()
        elif self.method == 'random_with_selective_memory':
            self.random_wSMemory()
    ...
    def random(self):
        nodes_tested = np.random.choice(self.graph_u, self.budget, replace = False)
    ...
    def random_wMemory(self):
        ...
        # visited.extend(testStrategy.get_result()[0])

    def random_wSMemory(self):
        raise NotImplementedError(f'The test strategy {self.method} is not defined.')
    ...
    def epsilon_memory(self):
        ...
        rec_pos = self.params['rec_pos']
        probs = np.random.random_sample((int(self.budget), ))
        p_memory = [i for i in probs if i >= epsilon]  # prob=epsilon to pick "random" in epsilon greedy
        # add random action with memory (randomly choose a node to test from those not visited)
        if len(p_memory) > len():
            pass
        p_eps = [i for i in probs if i < epsilon]
        # add optimal actions (random neighbor of rec_pos subset)
        ...
        if not isinstance(self.params['visited'], np.ndarray):
            self.params['visited'] = np.asarray(self.params['visited'])
        visited = self.params['visited']
        candidates = list(set(list(self.graph_u.nodes())) ^ set(visited)) #[n for n in self.graph_u.nodes() if n not in visited]
        size = len(probs)-len(candidates)
        if len(candidates) < size:
            nodes_tested = np.asarray(candidates)
        else:
            nodes_tested = np.random.choice(candidates, size, replace = False)
        nodes_tested = np.append(nodes_tested, candidates)
        self.update(nodes_tested)
        self.params['epsilon'] = max(epsilon - epsilon/(self.params['decay_factor']+0.001), 0)
        test_result = self.current_results[1]
        self.params['rec_pos'] = np.asarray([nodes_tested[i] for i in range(len(nodes_tested)) if test_result[i]=='I'])
        self.params['visited'] = np.append(self.params['visited'], nodes_tested)
